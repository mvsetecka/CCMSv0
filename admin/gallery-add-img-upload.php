...
if (is_uploaded_file($_FILES['Filedata']['tmp_name'])) {
	if (move_uploaded_file($_FILES['Filedata']['tmp_name'], $fn)){
        ...
        $query = "INSERT INTO galerie_obrazky(soubor, slozka, popis, velikost, zobrazeni, stav) VALUES ('".$_FILES['Filedata']['name']."', '".$gallery_id."', '', '".$_FILES['Filedata']['size']."', '0', '1')";
		$sql_query = new db_dotaz($query);
		$sql_query->proved_dotaz();
		$sql_query = NULL;
        
        $pictures_count = new db_dotaz("SELECT COUNT(id) AS pocet_obrazku FROM galerie_obrazky WHERE slozka = '".$gallery_id."'");
	    $pictures_count->proved_dotaz();
        $pocet_obrazu_temp = $pictures_count->get_vysledek();
        $pocet_obrazu = $pocet_obrazu_temp['pocet_obrazku'];
        $pictures_count = NULL;
        $pocet_obrazu_temp = NULL;
        
        $query = new db_dotaz("UPDATE galerie SET pocet_obrazu = '".$pocet_obrazu."' WHERE url = '".$gallery_id."'");
        $query->proved_dotaz();
        $query = NULL;
