<?php
if(isset($_POST['slozka_pict']) && $_POST['slozka_pict']<>"" && $_POST['slozka_pict']<>"..")
{
	$podadresar = $_POST['slozka_pict'];
	$adresar = "../media/images/".$podadresar."thumbs/";
	$top_form = 1;
}
elseif(isset($_POST['slozka_pict']) && $_POST['slozka_pict']=="..")
{
...
if($top_form == 1)
{
	...
	echo "<input type='submit' name='slozka_pict' value='..'>";
	echo "</form>";
}
$stream = dir($adresar);
echo "<form method='POST' action='".$_SERVER["PHP_SELF"]."?".$_SERVER['QUERY_STRING']."'>";
while($slozky=$stream->read())
{
	...
	{
		//echo $slozky."<br />";
		echo "<input type='submit' name='slozka_pict' value='".$slozky."/' style='background-color: White; border: none;' /><br />";
	}
}
echo "</form>";
$stream->close();
...
while ($fotka=$stream->read())
{
	...
	if (is_file($soubor))
	{
		...
		$preview_url = $soubor;
		$url = "media/images/".$podadresar.$fotka;

	?>
		
		<img src=<?php echo "'".$soubor."'"; ?> onclick="send_link(<?php echo "'".$url."', ''"?>);" width='66' height='50' alt='' class='img' style='margin:0px 0px 0px 0px' /><br />
